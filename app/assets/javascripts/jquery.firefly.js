...
(function($) {
	...
	$.firefly.create = function(img){
		...
		$('.fireflies').append(spark);
			return spark.css({
				'position':'absolute',
				top: $.firefly.random(($('.fireflies').height()-150)),	//offsets
				left: $.firefly.random(($('.fireflies').width()-150))		//offsets
				}).show();
}
...
$.firefly.fly = function(sp) {
	$(sp).animate({
		top: $.firefly.random(($('.fireflies').height()-150)),	//offsets
		left: $.firefly.random(($('.fireflies').width()-150))
	}, (($.firefly.random(10) + 10) * 4500),function(){ $.firefly.fly(sp) } );
};
